...
export default class BorrowLink extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      visible: false,
  ...
  handleOk() {
    ...
    this.refs.BorrowForm.validateFields((errors, values) => {
      if (!!errors) {
  ...
  render() {
    ...
    return (
      <span>
        <a onClick={this.showModal} className="btn btn-info btn-xs" >借出</a>
        <Modal
          maskClosable={false}
          title="档案借出"
          style={{ top: 20 }}
