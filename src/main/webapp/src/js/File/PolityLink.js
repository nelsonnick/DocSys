...
export default class PolityLink extends React.Component {
  ...
  handleOk() {
    ...
    this.refs.PolityForm.validateFields((errors, values) => {
      ...
      $.ajax({
        'type': 'POST',
        'url': AjaxFunction.PrintPolity,
        'dataType': 'text',
        'data': {
          'fid': values.fileId,
          'pid': values.personId,
          'pnation': values.personNation,
          'plearn': values.personLearn,
          'pface': values.personFace,
          'ptime': values.personTime,
          'pwork': values.personWork,
          'pzl': values.personZL,
          'pwg': values.personWG,
          'pfl': values.personFL,
        },
  ...
  render() {
    const { fileId, personId, personName, personNumber, personSex, personBirth } = this.props;
    return (
      <span>
        <a onClick={this.showModal} className="btn btn-warning btn-xs" >政审</a>
        <Modal
          maskClosable={false}
          title="打印政审材料"
          style={{ top: 20 }}
